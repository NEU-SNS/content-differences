...
results_file = "/tmp/resultsDetailed/numpagesresults.txt"
count_file = "/tmp/resultsDetailed/numpagescount.txt"
...
def process_log_file(summarylines, dname):
    for line in summarylines:
        if " from a set of " in line.decode():
            with open(results_file, 'a') as resultsf:
                fcntl.flock(resultsf, fcntl.LOCK_EX)
                resultsf.write(dname + " *** " +  line.decode().split(" from a set of ")[1].strip() + "\n")
