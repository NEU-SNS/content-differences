...
# Time in seconds to let parallel processes continue running before switching to a different batch (not used)
# BATCH_RUN_TIME = 2
...
# Experiment type
experimentType = "inDepth-allPhases"

# List of sites
allDomains = fetch_domains("alexa", 1000000)
...
if __name__ == "__main__":
    ...
    tstart = datetime.now()
    blacklist = ["northerntool.com"]
    for i in range(0, numDomainsProcess):
        if blacklist[0] in allDomains[i].decode():
                continue
